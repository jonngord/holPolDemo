...
# Create folder 
if ( ! dir.exists(paste0(folder_save, "/evenness"))) {
  dir.create(paste0(folder_save, "/evenness"), showWarnings = FALSE)
}

folder_save <- ("data_processed/arima_preds/evenness")
...
temp <- foreach(result = 1:length(diversity_data)) %dopar% {
  ...
  # Isolate evenness
  evenness <- div[["global_pollen_evenness"]]
  ...
  # Bin evenness data into 200 year bins
  evenness_bins <- evenness %>%
    # add time bins
    # 0 - 200 = bin 0, 
    # 201 - 400 = bin 1... etc
    dplyr::mutate(bin = floor(age_draw / 200) * 200) %>% 
    dplyr::group_by(bin) %>% 
    dplyr::summarise(bin_mean = mean(evenness, na.rm = TRUE)) %>% 
    dplyr::mutate(bin_age_centre = seq(100, 11700, 200)) %>% 
    dplyr::relocate(bin_age_centre, everything()) %>% 
    dplyr::rename(mean_evenness = bin_mean) %>% 
  ...
  # Join radiocarbon/climate data with evenness-curtis data 
  evenness_climate_radiocarbon_all <- 
    dplyr::left_join(evenness_bins, clim_spd,
                     by = "bin_age_centre") 
  ...
  evenness_clim <- evenness_climate_radiocarbon_all %>% 
    dplyr::arrange(desc(bin_age_centre)) %>% 
    dplyr::slice(- nrow(evenness_climate_radiocarbon_all))
  
  # Logit transformation of outcome variable, diversity 
  evenness_clim$evenness_logit <- car::logit(evenness_clim$mean_evenness) 
  
  # With predictor variables
  whole_hol <- as.matrix(
    cbind(evenness_clim[, c(4:19,      # gmst
                            31:41,     # prec
                            43:53)])   # C14
  )
  ...
  # 16 temp, 11 prec, 10 human.
  # All models must incude at least temp, prec and C14 (non-lagged)
  ...
  for (i in 1:nrow(truth_table)) {        # all combinations of ranges
    ...
    arima_fit_whole <- auto.arima(evenness_clim[, "evenness_logit"], 
                                  xreg = sub_mat,
                                  seasonal = FALSE)
  ...
  for (mod in (top_mods)) {
    ...
    fcast_whole <- fcast_whole %>% 
      ...
      # bind with ages
      dplyr::bind_cols(evenness_clim$bin_age_centre) %>% 
      dplyr::rename(age = 9)
    ...
    whole_hol_preds <- 
      ...
      dplyr::rename(bin_age_centre = V1,
                    evenness_forecast = V2,
                    evenness_forecast_95 = V3,
                    evenness_forecast_05 = V4)
    ...
    empirical <- evenness_bins %>% 
      dplyr::select(bin_age_centre, mean_evenness) %>% 
      dplyr::arrange(desc(bin_age_centre)) %>% 
    ...
    intersection_df <- dplyr::bind_cols(
      empirical,
      whole_hol_preds$evenness_forecast,
      whole_hol_preds_no_clim$evenness_forecast,
      whole_hol_preds_no_hum$evenness_forecast,
      whole_hol_preds_prec_only$evenness_forecast,
      whole_hol_preds_temp_only$evenness_forecast
    ) %>% 
      dplyr::rename(
        empirical_data = 2,
  ...
  ## Set filesave
  file_save <- file.path(folder_save, paste0("evenness_arima_preds_resample_", 
                                             result, 
                                             ".RDS"))
