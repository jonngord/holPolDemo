...
# Create folder 
if ( ! dir.exists(paste0(folder_save, "/richness"))) {
  dir.create(paste0(folder_save, "/richness"), showWarnings = FALSE)
}

folder_save <- ("data_processed/arima_preds/richness")
...
# Make list of all  diversity resamples
diversity_data <- as.list(dir("data_processed/diversity_results/richness_evenness_results", 
                              pattern = ".RDS", 
                              full.names = TRUE))

# Setup parallel backend
cores = detectCores()
clust <- parallel::makeCluster((cores/2) - 1) # this calculation = 3 cores on my machine
...
temp <- foreach(result = 1:length(diversity_data)) %dopar% {
  ...
  # Isolate richness
  richness <- div[["global_pollen_richness"]]
  ...
  # Bin richness data into 200 year bins
  richness_bins <- richness %>%
    # add time bins
    # 0 - 200 = bin 0, 
    # 201 - 400 = bin 1... etc
    dplyr::mutate(bin = floor(age_draw / 200) * 200) %>% 
    dplyr::group_by(bin) %>% 
    dplyr::summarise(bin_mean = mean(richness, na.rm = TRUE)) %>% 
    dplyr::mutate(bin_age_centre = seq(100, 11700, 200)) %>% 
    dplyr::relocate(bin_age_centre, everything()) %>% 
    dplyr::rename(mean_richness = bin_mean) %>% 
  ...
  richness_climate_radiocarbon_all <- 
    dplyr::left_join(richness_bins, clim_spd,
                     by = "bin_age_centre") 
  ...
  rich_clim <- 
    ...
    dplyr::slice(- nrow(richness_climate_radiocarbon_all))
  
  head(rich_clim)
  
  # With predictor variables
  whole_hol <- as.matrix(
    cbind(rich_clim[, c(4:19,      # gmst
                        31:41,     # prec
                        43:53)])   # C14
  )
  ...
  for (i in 1:nrow(truth_table)) {        # all combinations of ranges
    ...
    arima_fit_whole <- auto.arima(rich_clim[, "mean_richness"], 
                                  xreg = sub_mat,
                                  seasonal = FALSE)
  ...
  for (mod in (top_mods)) {
    ...
    fcast_whole <- fcast_whole %>% 
      tibble::as_tibble() %>% 
      dplyr::mutate(forecast_richness = `Point Forecast`, 
                    quant_95 = `Hi 95`,
                    quant_05 = `Lo 95`) %>% 
      # bind with ages
      dplyr::bind_cols(rich_clim$bin_age_centre) %>% 
      dplyr::rename(age = 9)
    
    whole_hol_preds <- 
      cbind(
        fcast_whole$age,
        fcast_whole$forecast_richness,
        fcast_whole$quant_95,
        fcast_whole$quant_05) %>% 
      as.data.frame() %>% 
      dplyr::rename(bin_age_centre = V1,
                    richness_forecast = V2,
                    richness_forecast_95 = V3,
                    richness_forecast_05 = V4)
    ...
    whole_hol_preds_no_hum <- 
      cbind(
        ...
        fcast_whole_no_hum$forecast_richness,
    ...
    whole_hol_preds_no_clim <- 
      cbind(
        ...
        fcast_whole_no_clim$forecast_richness,
    ...
    whole_hol_preds_temp_only <- 
      cbind(
        ...
        fcast_whole_temp_only$forecast_richness,
    ...
    whole_hol_preds_prec_only <- 
      cbind(
        ...
        fcast_whole_prec_only$forecast_richness,
    ...
    empirical <- richness_bins %>% 
      dplyr::select(bin_age_centre, mean_richness) %>% 
      dplyr::arrange(desc(bin_age_centre)) %>% 
    ...
    intersection_df <- dplyr::bind_cols(
      empirical,
      whole_hol_preds$richness_forecast,
      whole_hol_preds_no_clim$richness_forecast,
      whole_hol_preds_no_hum$richness_forecast,
      whole_hol_preds_prec_only$richness_forecast,
      whole_hol_preds_temp_only$richness_forecast
    ) %>% 
      dplyr::rename(
        empirical_data = 2,
  ...
  ## Set filesave
  file_save <- file.path(folder_save, paste0("richness_arima_preds_resample_", 
                                             result, 
                                             ".RDS"))
